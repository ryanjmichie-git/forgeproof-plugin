...
class TestV110Compat:
    """The checked-in fixture bundle was generated by the unmodified v1.1.0
    engine. Any `.rpack` ever signed must verify with every future version of
    the verifier — this test must never be weakened and the fixture must never
    be regenerated. If a change breaks it, the change is wrong.
    """
    ...
    def _deploy(self, tmp_path):
        ...
        chain_dir = tmp_path / ".forgeproof"
        chain_dir.mkdir()
        shutil.copyfile(FIXTURE_V110 / "chain-998.json", chain_dir / "chain-998.json")
        shutil.copyfile(FIXTURE_V110 / "issue-998.rpack", chain_dir / "issue-998.rpack")
        src = tmp_path / "src"
        src.mkdir()
        shutil.copyfile(FIXTURE_V110 / "src" / "example2.py", src / "example2.py")
        return chain_dir / "issue-998.rpack"

    def test_fixture_files_present(self):
        assert (FIXTURE_V110 / "chain-998.json").exists()
        assert (FIXTURE_V110 / "issue-998.rpack").exists()
        assert (FIXTURE_V110 / "src" / "example2.py").exists()
    ...
    def test_fixture_is_byte_exact(self):
        ...
        and chain_hash covers the text the engine sees via read_text() — with
        LF bytes on disk the two views are identical on every platform."""
        for rel in ("chain-998.json", "issue-998.rpack", "src/example2.py"):
            assert b"\r" not in (FIXTURE_V110 / rel).read_bytes(), (
                f"fixture file {rel} gained CR bytes (EOL conversion?)"
            )
        bundle = json.loads((FIXTURE_V110 / "issue-998.rpack").read_bytes())
        [artifact] = bundle["artifacts"]
        assert artifact["path"] == "src/example2.py"
        actual = fp.sha256_file(FIXTURE_V110 / "src" / "example2.py")
        assert actual == artifact["sha256"], (
            "fixture artifact bytes changed on disk"
        )
        chain_text = (FIXTURE_V110 / "chain-998.json").read_bytes().decode("utf-8")
        assert fp.sha256_hex(chain_text) == bundle["chain_hash"], (
            "fixture chain file bytes changed on disk"
        )

    def test_v110_bundle_verifies(self, tmp_path, monkeypatch, capsys):
        self._require_sshkeygen()
        rpack = self._deploy(tmp_path)
        monkeypatch.chdir(tmp_path)
        code, output = self._verify(rpack, capsys)
    ...
    def test_v110_tampered_artifact_fails(self, tmp_path, monkeypatch, capsys):
        self._require_sshkeygen()
        rpack = self._deploy(tmp_path)
        artifact = tmp_path / "src" / "example2.py"
        data = bytearray(artifact.read_bytes())
        data[0] ^= 0xFF  # flip one byte
        artifact.write_bytes(bytes(data))
        monkeypatch.chdir(tmp_path)
    ...
    def test_v110_tampered_chain_fails(self, tmp_path, monkeypatch, capsys):
        self._require_sshkeygen()
        rpack = self._deploy(tmp_path)
        chain_file = tmp_path / ".forgeproof" / "chain-998.json"
        chain = json.loads(chain_file.read_text())
        chain[2]["data"]["choice"] = "history, rewritten"
        chain_file.write_text(json.dumps(chain, indent=2) + "\n")
        monkeypatch.chdir(tmp_path)
    ...
    def test_v110_tampered_signature_fails(self, tmp_path, monkeypatch, capsys):
        """Exercises the Ed25519 failure branch specifically: the signature
        field is excluded from the root digest, so corrupting it leaves the
        digest intact — only real signature verification can catch it. A
        verify_signature that false-positives makes this test fail."""
